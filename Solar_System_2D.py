...
class Solar_system:
   ...
   def solve_acceleratin(self):
      total_gravity = np.sum(self.gravity, axis = (1))
      self.acceleration = np.divide(total_gravity, self.mass2[:, np.newaxis])
   ...
   def animation_planets(self, size):
      ...
      Solar_animation = animation.FuncAnimation(fig, func = animation_sol, frames = np.arange(0, len(self.Time_position), 1), interval = 13)
      plt.show()
